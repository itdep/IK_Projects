...
global $post, $woocommerce;
	...
	if ($attachments) {
		...
		foreach ( $attachments as $key => $attachment ) {
			...
			$h = 90;
			$crop = "true";
			$image_thumb = @vt_resize('' , wp_get_attachment_url( $attachment->ID ), $w, $h, $crop );	 

			$imagesHTML .=  '<li><a class="imgeffect magnifier" href="'.wp_get_attachment_url( $attachment->ID ).'" data-gal="prettyPhoto[rt_theme_products]" title="'.esc_attr( $attachment->post_title) .'"><img src="'.$image_thumb['url'].'" width="'.$image_thumb['width'].'" alt="'.esc_attr( $post->post_title) .'" /></a></li>';
